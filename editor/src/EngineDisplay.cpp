...
void Geodash3::Engine::m_Display()
{
	...
	{
		GL_CALL(glUniformMatrix4fv(m_button.GetUniformLocation("u_AspectMat"), 1, false, glm::value_ptr(glm::scale(glm::mat4(1.0f), glm::vec3(1.0f, (float)winW / (float)winH, 1.0f)))));	
	}	
	else if((float)winW <= (float)winH * 1920.0f / 1080.0f)
	{	
		GL_CALL(glUniformMatrix4fv(m_button.GetUniformLocation("u_AspectMat"), 1, false, glm::value_ptr(glm::scale(glm::mat4(1.0f), 1920.0f / 1080.0f * glm::vec3((float)winH / (float)winW, 1.0f, 1.0f)))));	
	}
	...
	m_modelViewMat = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -0.12f)) *  
					 glm::scale(glm::mat4(1.0f), glm::vec3(0.015f, 0.015f, 1.0f));
	GL_CALL(glUniformMatrix4fv(m_basic3D.GetUniformLocation("u_ModelViewMat"), 1, false, glm::value_ptr(m_modelViewMat)));	
	GL_CALL(glUniform1i(m_button.GetUniformLocation("u_hovering"), false));	
	...
	{	
		...
		m_modelViewMat = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -0.12f)) *  
					 glm::scale(glm::mat4(1.0f), glm::vec3(1.0f, 1.0f * 1080.0f / 1920.0f, 1.0f));
		GL_CALL(glUniformMatrix4fv(m_basic3D.GetUniformLocation("u_ModelViewMat"), 1, false, glm::value_ptr(m_modelViewMat)));
		GL_CALL(glDrawArrays(GL_TRIANGLES, 0, 6));
	}
	...
	else if(this->m_paused)
	{
		...
		GL_CALL(glDrawArrays(GL_TRIANGLES, 0, 6));
		//Discard changes
		GL_CALL(this->m_discardChanges.ActivateButtonTex());
		this->m_discardChanges.SetModelViewMat(this->m_modelViewMat);
		GL_CALL(glUniformMatrix4fv(m_button.GetUniformLocation("u_ModelViewMat"), 1, false, glm::value_ptr(m_modelViewMat)));	
		GL_CALL(glUniform1i(m_button.GetUniformLocation("u_hovering"), this->m_discardChanges.MouseHovering(this->m_gameWindow, this->m_mouseX, this->m_mouseY)));	
		GL_CALL(glDrawArrays(GL_TRIANGLES, 0, 6));	
		//Save & quit	
		...
		GL_CALL(glDrawArrays(GL_TRIANGLES, 0, 6));
		//return to menu
		GL_CALL(this->m_returnToMenu.ActivateButtonTex());
		this->m_returnToMenu.SetModelViewMat(this->m_modelViewMat);
		GL_CALL(glUniformMatrix4fv(m_button.GetUniformLocation("u_ModelViewMat"), 1, false, glm::value_ptr(m_modelViewMat)));	
		GL_CALL(glUniform1i(m_button.GetUniformLocation("u_hovering"), this->m_returnToMenu.MouseHovering(this->m_gameWindow, this->m_mouseX, this->m_mouseY)));	
		GL_CALL(glDrawArrays(GL_TRIANGLES, 0, 6));
		//Pull up help menu	
		...
		GL_CALL(glDrawArrays(GL_TRIANGLES, 0, 6));
		//Export	
		GL_CALL(this->m_exportButton.ActivateButtonTex());
		this->m_exportButton.SetModelViewMat(this->m_modelViewMat);
		GL_CALL(glUniformMatrix4fv(m_button.GetUniformLocation("u_ModelViewMat"), 1, false, glm::value_ptr(m_modelViewMat)));	
		GL_CALL(glUniform1i(m_button.GetUniformLocation("u_hovering"), this->m_exportButton.MouseHovering(this->m_gameWindow, this->m_mouseX, this->m_mouseY)));	
		GL_CALL(glDrawArrays(GL_TRIANGLES, 0, 6));	
		//Unexport	
		GL_CALL(this->m_unexportButton.ActivateButtonTex());
		this->m_unexportButton.SetModelViewMat(this->m_modelViewMat);
		GL_CALL(glUniformMatrix4fv(m_button.GetUniformLocation("u_ModelViewMat"), 1, false, glm::value_ptr(m_modelViewMat)));	
		GL_CALL(glUniform1i(m_button.GetUniformLocation("u_hovering"), this->m_unexportButton.MouseHovering(this->m_gameWindow, this->m_mouseX, this->m_mouseY)));	
		GL_CALL(glDrawArrays(GL_TRIANGLES, 0, 6));		
	}
