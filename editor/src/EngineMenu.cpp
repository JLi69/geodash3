...
{	
	...
	while(!glfwWindowShouldClose(this->m_gameWindow) && this->m_menu)
	{
		...
		if(m_pathIndex != 0)
		{
			GL_CALL(m_deleteButton.ActivateButtonTex());
			GL_CALL(m_deleteButton.SetModelViewMat(this->m_modelViewMat));
			GL_CALL(glUniformMatrix4fv(m_button.GetUniformLocation("u_ModelViewMat"), 1, false, glm::value_ptr(this->m_modelViewMat)));
			GL_CALL(glUniform1i(m_button.GetUniformLocation("u_hovering"), this->m_deleteButton.MouseHovering(this->m_gameWindow, this->m_mouseX, this->m_mouseY)));
			GL_CALL(glDrawArrays(GL_TRIANGLES, 0, 6));
		}
		...
		{
			GL_CALL(m_cancelDelete.ActivateButtonTex());
			GL_CALL(m_cancelDelete.SetModelViewMat(this->m_modelViewMat));
			GL_CALL(glUniformMatrix4fv(m_button.GetUniformLocation("u_ModelViewMat"), 1, false, glm::value_ptr(this->m_modelViewMat)));
			GL_CALL(glUniform1i(m_button.GetUniformLocation("u_hovering"), this->m_cancelDelete.MouseHovering(this->m_gameWindow, this->m_mouseX, this->m_mouseY)));
			GL_CALL(glDrawArrays(GL_TRIANGLES, 0, 6));	
		
			GL_CALL(m_confirmDelete.ActivateButtonTex());
			GL_CALL(m_confirmDelete.SetModelViewMat(this->m_modelViewMat));
			GL_CALL(glUniformMatrix4fv(m_button.GetUniformLocation("u_ModelViewMat"), 1, false, glm::value_ptr(this->m_modelViewMat)));
			GL_CALL(glUniform1i(m_button.GetUniformLocation("u_hovering"), this->m_confirmDelete.MouseHovering(this->m_gameWindow, this->m_mouseX, this->m_mouseY)));
			GL_CALL(glDrawArrays(GL_TRIANGLES, 0, 6));
		}
